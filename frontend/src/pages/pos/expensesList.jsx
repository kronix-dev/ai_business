...
export default function ExpensesList({ columns = [], rows = [] }) {
  const [cats, setCats] = React.useState([]);
  const [data, setData] = React.useState([]);
  const [open, setOpen] = React.useState(false);
  const [, forceUpdate] = React.useState();
  const getCats = () => {
    Budgeting.getExpenseCategories().then((r) => {
      setCats(r.data);
    });
  };
  ...
  return (
    <div>
      ...
      <KModal
        ...
        open={open}
        onOk={() => {
          Budgeting.createExpenditure(formData);
          setOpen(false);
          let p = data
          let f = formData
      ...
      >
        <Typography.Title level={4}>Add a expenditure record</Typography.Title>
        <KForm
          onChange={(data) => {
            setData(data);
          }}
